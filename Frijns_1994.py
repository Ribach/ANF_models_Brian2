...
if plot_voltage_course_lines | plot_voltage_course_colored:
    ...
    if plot_voltage_course_lines:
        plot.voltage_course_lines(plot_name = "Voltage course Frijns 1994",
                                  time_vector = M.t,
                                  voltage_matrix = M.v,
    ...
    if plot_voltage_course_colored:
        plot.voltage_course_colors(plot_name = "Voltage course Frijns 1994 (2)",
                                   time_vector = M.t,
                                   voltage_matrix = M.v,
                                   distance_comps_middle = data.distance_comps_middle)
...
if measure_single_node_response:
    ...
    ##### current amplitude
    current_amps = np.array([-3, -4, -5, -10, -50])*nA
    ...
    for ii in range(0, len(current_amps)):
        ...
        unit = r'$nV$'
        plt.text(0.04, 100*ii+V_res/mV+10, f"{round(current_amps[ii]*10**9/amp,1)} {unit}")
        ...
        for jj in range(0,nof_runs):
            ...
            node_response_data["stimulation amplitude (nA)"][ii*nof_runs+jj] = current_amps[ii]/nA
