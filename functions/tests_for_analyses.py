...
def get_refractory_periods_for_pulse_trains(model_name,
    ...
    while inter_pulse_gap_diff > delta:
        ...
        # print progress
        if print_progress: print("ARP: Model: {} us, Pulse rate: {}, Inter pulse gap: {} us".format(model_name, pulse_rate, np.round(inter_pulse_gap/us)))
        ...
        nof_spikes = len(peak.indexes(M.v[comp_index,M.t > runtime_masker+inter_pulse_gap], thres = model.V_res + 60*mV, thres_abs=True))
    ...
    while inter_pulse_gap_diff > delta:
        ...
        # print progress
        if print_progress: print("RRP: Phase duration: {} us, Inter pulse gap: {} us".format(np.round(phase_durations[0]/us), np.round(inter_pulse_gap/us)))
